...
function set-address(){
    ...
    $adapter = Get-NetAdapter | ? {$_.Name -eq $interface}
    ...
    if ($yon -eq "yes" -or $yon -eq "y"){
        ...
        Set-NetIPAddress -InterfaceIndex $adapter.InterfaceIndex -IPAddress $newIP -PrefixLength $newMask -AddressFamily ipv4
    } else {
        set-address
    }
}
